function TextAnimated(slot, deltapixel, deltatime, deltadirection, title, titlefont, titlefillstyle, padding, paddingx, paddingy)
{
    ...
    this.Draw = function (surface) {
        ...
        if (stopAnimation) {
            ...
            if (this.DeltaDirection == "FromBottom") {
                //surface.translate(this.X() + (metrics.width / 2) + this.PaddingX, this.Y() + this.Height() + this.PaddingY);
                surface.translate(this.X() + this.PaddingX, this.Y() + this.Height() + this.PaddingY);
                surface.fillText(this.Title, 0, 0, this.Width());
            }
            else if (this.DeltaDirection == "FromTop") {
                //surface.translate(this.X() + (metrics.width / 2) + this.PaddingX, this.Y() + this.PaddingY);
                surface.translate(this.X() + this.PaddingX, this.Y() + this.PaddingY);
                surface.fillText(this.Title, 0, 0, this.Width());
            } else if (this.DeltaDirection == "FromLeft") {
                surface.translate(this.X() + this.PaddingX, this.Y() + (metrics.width / 2) + this.PaddingY);
                surface.rotate(90 * (Math.PI / 180));
                surface.fillText(this.Title, 0, 0, this.Width());
            }
            else if (this.DeltaDirection == "FromRight") {
                surface.translate(this.X() + this.Width() + this.PaddingX, this.Y() + this.Height() - (metrics.width / 2) + this.PaddingY);
                surface.rotate(-90 * (Math.PI / 180));
                surface.fillText(this.Title, 0, 0, this.Width());
            }
            surface.restore();
